...
func TestAddBlock(t *testing.T) {
	...
	assert.NotNil(t, bc.AddBlock(randomBlock(uint32(89), types.Hash{})))
}

func TestAddBlockToHigh(t *testing.T) {
	bc := newBlockchainWithGenesis(t)
	assert.Nil(t, bc.addBlockWithoutValidation(randomBlockWithSignature(t, 1, getPrevBlockHash(t, bc, uint32(1)))))
...
func TestGetHeader(t *testing.T) {
	...
	lenBlocks := uint32(1000)
	for i := 0; i < int(lenBlocks); i++ {
		block := randomBlockWithSignature(t, uint32(i+1), getPrevBlockHash(t, bc, uint32(i+1)))
		assert.Nil(t, bc.AddBlock(block))
